...
function xScale(hData, chosenXAxis) {
  ...
  var xLinearScale = d3.scaleLinear()
    .domain([d3.min(hData, d => d[chosenXAxis]) *0.5 ,
      d3.max(hData, d => d[chosenXAxis]) * 1
    ])
    .range([0, width]);
...
function renderCircles(circlesGroup, newXScale, chosenXaxis, textGroup) {
  ...
  circlesGroup.transition()
    ...
    .attr("cx", d => newXScale(d[chosenXAxis]));
     ...
     textGroup.transition()
                    .duration(1000)
                    .attr("y",  d => newScale(d.healthcare));
  return circlesGroup;
}
//
...
d3.csv("data.csv").then (function(healthcare) {
  ...
  // parse data
  healthcare.forEach(function(data,index) {
  ...
  var xLinearScale = xScale(healthcare, chosenXAxis);
...
//               .text(d => d.abbr);
  ...
  chartGroup.append("text")
    ...
    var circlesGroup = renderCircles(circlesGroup, xLinearScale, chosenXAxis,textGroup);
